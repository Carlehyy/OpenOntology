...
def test_execution_stream_contract_isolated_from_pipeline_stream():
    ...
    assert EXECUTION_CALL_SUBJECT == "sa.execution.call.*"
    ...
    assert {item[0] for item in registry} == {EXECUTION_RUN_SUBJECT, EXECUTION_CALL_SUBJECT, EXECUTION_RECONCILE_SUBJECT}
    assert {item[1] for item in registry} == {"sa-kernel-v1", "sa-call-v1", "sa-reconciler-v1"}
