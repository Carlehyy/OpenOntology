...
super_assistant_palace_sync_tokens（0102 建表守卫跳过）；0102 的建表 DDL
对存量库生效，且 downgrade 必须能干净移除。这里用「head → 降级 0100 →
再升 head」的往返验证两条路径。
"""
...
def test_upgrade_downgrade_roundtrip(tmp_path, monkeypatch):
    ...
    # 降级到 0100：表被移除（此时库不再有 0100 的痕迹）
    command.downgrade(cfg, "0100_super_assistant_tool_settings")
    engine = create_engine(f"sqlite:///{db_path}")
    assert "super_assistant_palace_sync_tokens" not in inspect(engine).get_table_names()
    engine.dispose()

    # 再升回 head：这次由 0102 自己的 create_table 建表（存量库路径）
    command.upgrade(cfg, "head")
    engine = create_engine(f"sqlite:///{db_path}")
