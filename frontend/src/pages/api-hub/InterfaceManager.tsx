                ...
                return (
                  <div key={index} className="rounded-lg border border-border bg-muted p-2.5">
                    <div className="flex flex-wrap items-center gap-2">
                      <input aria-label={`第 ${index + 1} 个文件字段名`} value={field.key} onChange={event => updateFileField(index, { key: event.target.value })} className="h-8 min-w-[110px] flex-1 rounded-md border border-border bg-card px-2.5 font-mono text-xs outline-none" placeholder="字段名，例如 file" />
                      <input aria-label={`${field.key || `第 ${index + 1} 个字段`}允许的文件类型`} value={field.accept} onChange={event => updateFileField(index, { accept: event.target.value })} className="h-8 min-w-[150px] flex-[1.3] rounded-md border border-border bg-card px-2.5 font-mono text-[11px] outline-none" placeholder=".pdf,image/*（可选）" />
