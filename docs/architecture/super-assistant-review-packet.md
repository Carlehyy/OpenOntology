...
当前实现门槛应按以下事实读取：M1 的状态/事件/幂等/租约、heartbeat、stuck 扫描和 parent fan-in 已有实现与专项测试，但跨进程 heartbeat 和复杂子结果归并仍需 staging 证据；M2 的执行表、迁移、Outbox、NATS 基础、历史显式回填/回滚和 DLQ 重放入口已实现，真实 PostgreSQL/NATS 验收仍未闭环；M3 已具备可持续多步 activation、等待输入/审批/外部结果、恢复、Artifact 结果证据，以及通过 `sa.execution.call.<owner>` durable consumer 进入 ConnectorRegistry 的直连外部 Call 派发；连接器现在会按 owner 解析，并在首个调用前冻结 `CapabilityRevision`，不可用或异常进入 `outcome_unknown/reconciling`，但 Connector 的异步进度/取消/重试、pull 传输和远端取消仍需补齐；M4 已有不可变 Capability/Connector、Remote Agent HTTP 适配、MCP 工具 Connector、首调快照和 MCP manifest 变更/禁用/卸载撤销、`plugin_host.py` 的独立 JSON-lines 进程边界，以及回调身份校验，Multica 的远端查询/取消、插件持久化安装和 Kernel 调度挂接仍未闭环；M5 已接入 Memory/Palace ContextSource、来源字段、tombstone 持久化和索引排除，Artifact 对象存储仍未接入；M6 已有 kernel.v1 API/SSE、输入/审批/Artifact、深链、失败重试 API、多 Run 列表/任务卡、410 游标恢复和内联 Artifact 下载，事件细粒度实时投影和非内联对象存储下载仍需补齐；M7/M8 的真实 staging 验收和最终发布证据尚未完成。
