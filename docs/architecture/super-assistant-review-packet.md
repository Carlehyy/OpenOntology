...
## 11. 当前实现证据（2026-09-14，基线提交 `2c8ef2dc`）
...
新增专项回归为 `33 passed`（router/reconciler/recovery/callback/process-plugin），前端静态门禁和 unit/build 通过；当时直接复用已占用的跨 worktree Vite 服务运行 `test:e2e:mocked` 得到 `253 passed, 53 failed`，后续确认失败来自旧 bundle/错误端口而非产品断言。改用隔离端口后当前结果为 `306 passed`，并已将默认配置改为不隐式复用已有服务。独立 rootless plugin-runner/secret broker、真实 staging 外部副作用、迁移升级与回滚仍未完成，因此本轮审查不构成商用发布批准。
...
迁移报告运维入口已补齐脚本自举：在 `backend` 目录直接执行 `uv run python scripts/super_assistant_migration_report.py`（无需手工设置 `PYTHONPATH`）可输出 `kernel.v1.legacy-disposition.v1` 只读报告；`--help` 在无数据库配置时也可用。入口已补齐 ORM registry 初始化，避免独立 CLI 进程在 PostgreSQL flush 时缺少 `users` mapper。隔离 PostgreSQL 已实际验证带历史数据的 `apply → replay → rollback → replay`：首轮创建 1 个 legacy Run 和 2 个 CapabilityRevision，重复 apply 与回滚后重放均不重复创建，回滚只影响指定 migration id。
...
本轮又修复了 Runner 在 `unknown` 终态事件与 journal 状态之间的崩溃窗口：现在先提交可重放的 `unknown` 回执，再提交终态；若进程恰好在两次提交之间退出，重投会补齐状态并重放同一事件，不会追加第二条回执或静默丢失证据。Runner service、插件目录和 Connector 专项合计 `38 passed`，新增崩溃恢复回归覆盖该路径。
